...
pub enum Quota {
    ...
    MultiWindow { burst: GovernorQuota, sustained: GovernorQuota },
}

/// Specification for an endpoint's rate limits.
///
/// This is typically generated by the `#[rate_limit]` macro.
#[non_exhaustive]
#[derive(Clone, Debug)]
pub struct Spec {
    ...
    pub global_quota: Option<GovernorQuota>,
}
...
pub fn parse_quota_literal(quota: &str) -> (u32, &str) {
    let parts: Vec<&str> = quota.split('/').collect();
    assert!(parts.len() == 2, "Invalid quota format: expected 'count/period' (e.g., '1500/10s')");

    let count: u32 = parts[0]
        .parse()
        .unwrap_or_else(|_| panic!("Invalid count in quota: {}", parts[0]));
...
impl RateLimiters {
    ...
    pub async fn check_spec(&self, spec: &Spec) -> crate::Result<()> {
        // Check endpoint-specific limiter(s)
        match &spec.quota {
            Quota::Single(quota) => {
        ...
        // Check API-level limiter
        if let Some(api_quota) = spec.api_quota {
            let api_key = format!("{}_api", spec.key.split('_').next().unwrap_or("unknown"));
...
macro_rules! check {
    ...
    ($self:expr, key: $key:expr, quota: $quota:expr $(, api_quota: $api_quota:expr)? $(, global_quota: $global_quota:expr)?) => {{
        ...
        if let Some(ref limiters) = $self.rate_limiters {
            ...
            $(let global_quota = {
                let (count, period) = $crate::http::rate_limit::parse_quota_literal($global_quota);
                Some($crate::http::rate_limit::parse_quota_str(count, period))
            };)?
            let global_quota = None $( .or(Some({
                let (count, period) = $crate::http::rate_limit::parse_quota_literal($global_quota);
                $crate::http::rate_limit::parse_quota_str(count, period)
            })) )?;
    ...
    // Multi-window quota (burst + sustained)
    ($self:expr, key: $key:expr, burst: $burst:expr, sustained: $sustained:expr $(, api_quota: $api_quota:expr)? $(, global_quota: $global_quota:expr)?) => {{
        #[cfg(feature = "rate-limiting")]
        if let Some(ref limiters) = $self.rate_limiters {
