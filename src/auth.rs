...
/// Each configured client can exist in one state at a time, i.e. [`state::Unauthenticated`] or
/// [`state::Authenticated`].
pub mod state {
    ...
    /// The elevated state of the client.
    ///
    /// See `examples/clob/authenticated.rs` for more context.
    #[non_exhaustive]
    #[derive(Clone, Debug)]
